...
| 9 | `find_why_now_signals` | §7 new P0 | _not yet_ | P0 | ❌ |
| 10 | `estimate_demand_signals` | §7 new P0 | _not yet_ | P0 | ❌ |
| 11 | `find_public_revenue_signals` | §7 new P1 | _not yet_ | P1 | ❌ |
| 12 | `assess_platform_dependency` | §7 new P1 | _not yet_ | P1 | ❌ |
...
**Net remaining for v1 completion:** 4 tools (2 P0, 2 P1) per spec §10 Build Sequence.
...
**`src/lib/`:**
- Purpose: shared infrastructure used by tools — external API clients, in-memory cache, raw fetch helpers
- Contains: 6 modules; each exports a search/fetch function + `isXyzLive()` + `xyzSource()` helper + `xyzConfidenceNote()` helper
- Key files:
  - `src/lib/serper.ts` — Google search via Serper API (foundational; most tools use this)
  - `src/lib/reddit.ts` — Reddit signals via Serper site-restricted queries
...
**`src/tools/`:**
- Purpose: MCP tools — live-data fetchers returning `ToolResult<T>` with tier+bias-labeled sources
- Contains: 8 of 12 tools per spec §7
- Naming: kebab-case file = snake_case tool name (e.g. `find-pricing-anchors.ts` → `find_pricing_anchors`)
...
- Parallel naming: file basename matches the MCP-registered name with `_` ↔ `-` (e.g. `find_pricing_anchors` ↔ `find-pricing-anchors.ts`)
...
**New tool (e.g. completing P0 set — `find_why_now_signals`):**
- Primary code: `src/tools/find-why-now-signals.ts`
- Pattern: copy `src/tools/check-big-tech-encroachment.ts` (closest analog — Gate 5 primary, Serper-driven)
- Export: `registerFindWhyNowSignals(server)`
- Wire up: add import + call in `src/index.ts` (after line 26 import, after line 89 registration), update startup log on line 103
- Resource map: also add an entry in `src/resources/tool-to-gate-map.md` so the model sees it
- Return shape: `ToolResult<FindWhyNowSignalsData>` per `src/types.ts`
